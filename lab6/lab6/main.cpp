...
std::optional<Args> ParseArgs(int argc, char* argv[])
{
	if (argc < 6)
	{
		std::cout << "Usage: {lab6.exe} {input file name} {output file name} {threads number} {cores number} {array of priorities}\n";
		return std::nullopt;
	}
...
void BlurHorizonStripe(BlurParams* blurParams, int blurRadius)
{
	...
	for (int i = 0; i < blurParams->bmp->TellHeight(); i++)
	{
		for (int j = blurParams->stripeStart; j < blurParams->stripeEnd; j++)
		{
			...
			*blurParams->ofstm << std::to_string(clock()) + "\n";
			blurParams->logBuffer->Log((int)clock());
		}
	}
}
...
BlurParams* GetBlurParams(BMP& bmp, int threadsNumber, LogBuffer* logBuffer, std::ofstream* ofstrm)
{
	...
	for (int i = 0; i < threadsNumber - 1; i++)
	{
		...
		BlurParams stripeParams = { &bmp, logBuffer, ofstrm, currentWidth, nextWidth };
		blurParams[i] = stripeParams;
		currentWidth = nextWidth;
	}

	BlurParams stripeParams = { &bmp, logBuffer, ofstrm, currentWidth, currentWidth + lastStripeWidth };
...
void RunThreads(BMP& bmp, int threadsNumber, int coresNumber, const std::vector<int>& priorities)
{
	...
	std::ofstream* ofstrm = new std::ofstream("file-output.txt");
	BlurParams* blurParams = GetBlurParams(bmp, threadsNumber, logBuffer, ofstrm);
